...
本项目是 ECharts 的支付宝小程序版本，以及使用的示例。
...
`ec-canvas` 目录下有一个 `echarts.js`，默认我们会在每次 `echarts-Aliapp` 项目发版的时候替换成最新版的 ECharts。如有必要，可以自行从 ECharts 项目中下载[最新发布版](https://github.com/ecomfe/echarts/releases)，或者从[官网自定义构建](http://echarts.baidu.com/builder.html)以减小文件大小。

## 引入组件

在开始之前，你需要先阅读 [支付宝小程序自定义组件](https://docs.alipay.com/mini/framework/custom-component-overview) 的相关文档。


在创建项目之后，可以将下载的 [echarts-Aliapp](https://github.com/ant-move/echarts-Aliapp) 项目完全替换新建的项目，然后将修改代码；或者仅拷贝 `ec-canvas` 目录到新建的项目下，然后做相应的调整。
...
首先，在 `pages/bar` 目录下新建以下几个文件：`index.js`、 `index.json`、 `index.axml`、 `index.acss`。并且在 `app.json` 的 `pages` 中增加 `'pages/bar/index'`。
...
这一配置的作用是，允许我们在 `pages/bar/index.axml` 中使用 `<ec-canvas>` 组件。注意路径的相对位置要写对，如果目录结构和本例相同，就应该像上面这样配置。

`index.axml` 中，我们创建了一个 `<ec-canvas>` 组件，内容如下：
...
> 注意此处的 `.container`，新建小程序项目后，其中 `app.acss` 中默认自动生成的此 class 与本 demo 中的可能不一致，导致图表不能正常显示，只显示空白。请注意参考 `app.acss` 修改样式，保证图表的初始化的时候是有宽度和高度的。
...
完整的例子请参见 [echarts-Aliapp](https://github.com/ant-move/echarts-Aliapp) 项目。
...
目前，本项目已支持 ECharts Tooltip，但是由于 ECharts 相关功能尚未发版，因此需要使用当前本项目中 `ec-canvas/echarts.js`，这个文件包含了可以在支付宝小程序中使用 Tooltip 的相关代码。目前在 ECharts 官网下载的 `echarts.js` 还不能直接替换使用，等 ECharts 正式发版后即可。
...
以下功能尚不支持，如果有相关需求请在 [issue](https://github.com/ant-move/echarts-Aliapp/issues) 中向我们反馈，对于反馈人数多的需求将优先支持：
